...
def create_model(input_channels=0):
...
class Pointnet2MSG(nn.Module):
    def __init__(self, input_channels=6):
        ...
        for k in range(0, CLS_FC.__len__()):
            mlps = [nn.Conv1d(pre_channel, CLS_FC[k], kernel_size=(1,), stride=(1,), bias=False),
                    nn.BatchNorm1d(CLS_FC[k]),
                    nn.ReLU()]
            cls_layers.extend(mlps)
        cls_layers.append(nn.Conv1d(pre_channel, 1, kernel_size=(1,), stride=(1,), bias=False))
        cls_layers.insert(3, nn.Dropout(0.5))
